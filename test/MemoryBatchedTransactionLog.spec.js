...
describe("MemoryBatchedTransactionLog", () => {
  let log;

  beforeEach(() => {
    log = new MemoryBatchedTransactionLog();
  });
  ...
  it("'replay()' iterates over the transaction-log entries, calling the provided function", async () => {
    ...
    assert.strictEqual(fourth, moveThree);
  });
  ...
  it("'getSeqRangeCommits()' resolves to the commits within the given range", async () => {
    ...
    // assert.equal(first.transactions[0].commit, 2);
    // assert.equal(second.transactions[0].commit, 3);
